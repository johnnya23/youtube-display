 ...
 */
function jmayt_detect_shortcode($needles = array(), $post_item = 0)
{
    if ($post_item) {
...
if (is_array($jmayt_options_array))
...
function jma_yt_grid($atts)
{
    ...
    if (!$atts['id']) {
        return 'Enter a list id (or URL)';
    } elseif (substr($atts['id'], 0, 4) === "http") {
        $atts['id'] = substr($atts['id'], strpos($atts['id'], "list=") + strlen("list="));
    }
...
function jma_yt_video($atts)
{
    ...
    if (!isset($atts['id']) || !$atts['id']) {
        return 'please enter a video id (or URL)';
    } elseif (substr($atts['id'], 0, 4) === "http") {
        $atts['id'] = jmayt_id_from_url($atts['id']);
    }
    if (isset($atts['className'])) {
        $atts['class'] = $atts['className'];
    }
